...
def calculate_environmental_stats(cans, user_reported_count, user_volumes=None):
    ...
    # Použít ručně zadané objemy od uživatele, pokud existují a u plechovky chybí objem
    if user_volumes and isinstance(user_volumes, list):
        for i, can in enumerate(adjusted_cans):
            if can.get('volume_liters') == 'Unknown' or not can.get('volume_liters'):
                if i < len(user_volumes) and user_volumes[i] is not None:
                    can['volume_liters'] = user_volumes[i]
...
def main():
    ...
    try:
        ...
        query_url = f"{SUPABASE_URL}/rest/v1/pickups?is_analyzed=eq.false&photo_url=not.is.null&select=id,photo_url,count,nickname,user_volumes"
...
"""
            ...
            # Výpočet statistik
            stats = calculate_environmental_stats(detected_cans, pickup['count'], pickup.get('user_volumes'))
            print(f"  ⚖️ Výpočty:")
